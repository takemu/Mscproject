...
class TestFBAModelBaseline(unittest.TestCase):
    ...
    def test_pfba_baseline(self):
        fba_model = FBAModel(conditions=pd.DataFrame([[self.condition]]), p_fba=True)
        solution = fba_model.solve()
        solution.to_csv('pfba_small_baseline.csv')

    def test_pfba_batch_baseline(self):
        fba_model = FBAModel(conditions=pd.read_csv('../simulation/data/perturbations.csv'), p_fba=True)
        solution = fba_model.solve()
...
class TestFBAModel(unittest.TestCase):
    ...
    def test_comparison(self):
        ...
        pfba_data = FBAModel(conditions=pd.DataFrame([[condition]]), p_fba=True).solve()
    ...
    def test_similarity(self):
        ...
        for column in pfba_data.iloc[:, 0:]:
            ...
            similarity = diff[diff < 1e-6].count() / compare_df.shape[0]
