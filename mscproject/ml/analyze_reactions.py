...
def analyze(name='ptfa_mlp', model='iML1515', threshold=0):
    ...
    results.to_csv(f'output/results/{name}_results.csv')
    results['kegg.reaction'].to_csv(f'output/results/{name}_keggtxt', sep=' ', index=False, header=False)
    results['biocyc'].to_csv(f'output/results/{name}_biocyc.txt', sep=' ', index=False, header=False)


if __name__ == '__main__':
    analyze(name='yangs_lr', model='iJO1366', threshold=10)
    analyze(name='ptfa_lr', threshold=10)
    analyze(name='ptfa_mlp', threshold=10)
